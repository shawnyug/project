...
class MultiEncoderCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cross_attention = nn.MultiheadAttention(n_hidden, num_heads=4)
    ...
    def forward(self, x: List[torch.Tensor], M: List[torch.Tensor], head_id: int, *cat_list: int):
        """Forward pass."""
        # input x should be a list of tensors, contains both the scRNA and ST

        # if head_id == 0:
        #     q1 = self.encoders[0](x[0], *cat_list)
        #     q2 = self.encoders[1](x[1], *cat_list)
        #     q, _ = self.cross_attention(q1, q2, q2)
        #     q = q + q1
        #     #q = self.encoders[0](x[0], *cat_list)
        # else:
        #     q1 = self.encoders[0](x[0], *cat_list)
        #     q2 = self.encoders[1](x[1], *cat_list)
        #     q, _ = self.cross_attention(q2, q1, q1)
        #     q = q + q2
        #     #q = self.encoders[1](x[1], *cat_list)
            
        #     #q = self.norm(q)

        # attention score based on correlation matrix
        if head_id == 0:
            q1 = self.encoders[0](x[0], *cat_list)
            q2 = self.encoders[1](x[1], *cat_list)
            # q1 = self.encoder_shared(q1, *cat_list)
            # q2 = self.encoder_shared(q2, *cat_list)
            #q, _ = self.cross_attention(q1, q2, q2)
            #M_norm = torch.div(M[0], M[0].sum(dim=0, keepdim=True))
            M_norm = torch.nn.Softmax(dim=0)
            q = torch.matmul(M_norm(M[0]), q2)
            q = q + q1
            #q = self.encoders[0](x[0], *cat_list)
        else:
            q1 = self.encoders[0](x[0], *cat_list)
            ...
            # q2 = self.encoder_shared(q2, *cat_list)
            #q, _ = self.cross_attention(q2, q1, q1)
            #M_norm = torch.div(M[1], M[1].sum(dim=1, keepdim=True))
            M_norm = torch.nn.Softmax(dim=1)
            q = torch.matmul(torch.t(M_norm(M[1])), q1)
            q = q + q2
            #q = self.encoders[1](x[1], *cat_list)
